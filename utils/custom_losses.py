...
def rigid_from_3_points_iterative(x1, x2, x3):
    R_batch_list = []

    for sample_idx in range(len(x1)):
        R_sample_list = []
        x1_sample = x1[sample_idx]

        for res_idx in range(len(x1_sample)):
            v1 = x3[sample_idx][res_idx] - x2[sample_idx][res_idx]
            v2 = x1_sample[res_idx] - x2[sample_idx][res_idx]
            e1 = v1 / torch.norm(v1, keepdim=True)
            u2 = v2 - e1 * torch.sum(e1 * v2)
            e2 = u2 / torch.norm(u2, keepdim=True)
            e3 = torch.cross(e1, e2)

            R = torch.stack([e1, e2, e3], dim=1)
            R_sample_list.append(R)

        R_sample = torch.stack(R_sample_list)
        R_batch_list.append(R_sample)

    R_batch = torch.stack(R_batch_list)
    t = torch.clone(x2)

    return R_batch, t


def rigid_from_3_points_residue(x1, x2, x3):
    v1 = x3 - x2
    v2 = x1 - x2
    e1 = v1 / torch.norm(v1, keepdim=True)
    u2 = v2 - e1 * torch.sum(e1 * v2)
...
def fape_loss_iterative(x_predicted, x_true, epoch, Z=100.0, d_clamp=100.0, epsilon=1e-4):
    ...
    for i in range(batch_size):
        ...
        for j in range(num_amino_acids):
            x_true_res = x_true[i][j]
            x_pred_res = x_predicted[i][j]

            # Compute the rigid transformation using the first three amino acids
            r_true, t_true = rigid_from_3_points_residue(x_true_res[0], x_true_res[1], x_true_res[2])
            r_predicted, t_predicted = rigid_from_3_points_residue(x_pred_res[0], x_pred_res[1], x_pred_res[2])

            x_true_alpha_carbon = x_true_res[1].squeeze()
            x_predicted_alpha_carbon = x_pred_res[1].squeeze()

            # Transform all true and predicted points
            x_true_transformed = torch.einsum('ij,j->i', r_true, x_true_alpha_carbon)
            x_predicted_transformed = torch.einsum('ij,j->i', r_predicted, x_predicted_alpha_carbon)
            ...
            # Normalize by Z
            fape = clamped_distances / Z
            fape_sample_list.append(fape)

        fape_sample = torch.stack(fape_sample_list)
        fape_batch_list.append(fape_sample)

    fape_batch = torch.stack(fape_batch_list)
