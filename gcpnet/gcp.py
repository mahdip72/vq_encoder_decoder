    ...
    def scalarize(
        ...
        if node_mask is not None:
            ...
            local_scalar_rep_i = torch.zeros(
                (edge_index.shape[1], 3, 3), dtype=dtype, device=edge_index.device
            )
            local_scalar_rep_i[edge_mask] = torch.matmul(
                frames[edge_mask].type(dtype), vector_rep_i[edge_mask]
            )
...
class GCPEmbedding(nn.Module):
    ...
    def forward(
        ...
        edge_vectors = (
            batch.x[batch.edge_index[0]] - batch.x[batch.edge_index[1]]
        )  # [n_edges, 3]
