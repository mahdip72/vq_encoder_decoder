...
class VQVAETransformer(nn.Module):
    ...
    def forward(self, x, mask, nan_mask, **kwargs):
        if not self.decoder_only:
            ...
            x = self.encoder_blocks(x, mask=torch.logical_or(mask, nan_mask))
        ...
        if self.vqvae_enabled:
            if not self.decoder_only:
                ...
                x, indices, commit_loss = self.vector_quantizer(x, mask=torch.logical_or(mask, nan_mask))
        ...
        x = self.decoder(x, torch.logical_or(mask, nan_mask))
        return x, indices, commit_loss