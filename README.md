...
[![bioRxiv Preprint](https://img.shields.io/badge/bioRxiv-10.1101/2025.10.01.679833v1-brightgreen)](https://www.biorxiv.org/content/10.1101/2025.10.01.679833v3)
...
Converting protein tertiary structure into discrete tokens via vector-quantized variational autoencoders (VQ-VAEs) creates a language of 3D geometry and provides a natural interface between sequence and structure models. While pose invariance is commonly enforced, retaining chirality and directional cues without sacrificing reconstruction accuracy remains challenging. We introduce GCP-VQVAE, a geometry-complete tokenizer built around a strictly SE(3)-equivariant GCPNet encoder that preserves orientation and chirality of protein backbones. We vector-quantize rotation- and translation-invariant readouts that retain chirality into a 4,096-token vocabulary, and a Transformer decoder maps tokens back to backbone coordinates via a 6D rotation head trained with SE(3)-invariant objectives. We train GCP-VQVAE on 24 million monomer protein backbone structures from the AlphaFold Protein Structure Database. On CAMEO2024, CASP15, and CASP16, the model achieves backbone RMSDs of 0.4377 Å, 0.5293 Å, and 0.7567 Å, respectively, and reaches 100% codebook utilization, substantially outperforming prior VQ-VAE-based tokenizers. On a zero-shot set of 1,938 newly deposited experimental structures, GCP-VQVAE attains a backbone RMSD of 0.8193 Å and a TM-score of 0.9673, demonstrating robust generalization to unseen proteins. Finally, the Large and Lite variants are approximately 408× and 530× faster than the previous state of the art (AIDO), while remaining robust to structural noise. We make the GCP-VQVAE source code, zero-shot dataset, and its pretrained weights fully open for the research community.
...
- 🗓️ **3 Oct 2025** — Our preprint has been published in [bioRxiv](https://www.biorxiv.org/content/10.1101/2025.10.01.679833v3).
...
@article{Pourmirzaei2025gcpvqvae,
  ...
  url     = {https://www.biorxiv.org/content/10.1101/2025.10.01.679833v3}
}
```
