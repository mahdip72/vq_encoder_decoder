...
def build_dataset(split: str, args, tokenizer, logger):
    ...
    ds = InterProFunctionDataset(
        ...
        split=split,
        target_field="conservedsite_label",
        pdb_data_dir=None,
        tokenizer=tokenizer,
        logger=logger,
        cache=True,
...
def parse_args():
    p = argparse.ArgumentParser(description="InterPro conserved site AUROC eval (minimal)")
    p.add_argument(
        "--h5",
        required=True,
        help="Path to InterPro conserved-site H5 (e.g., vq_embed_conserved_lite.h5)",
    )
    p.add_argument(
        "--embeddings-dataset",
        default="/",
...
def main():
    ...
    logger = logging.getLogger("interpro_conserved_eval")
