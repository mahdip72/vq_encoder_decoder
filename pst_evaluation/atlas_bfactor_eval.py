...
def eval_spearman(model, loader, device, show_progress=False, desc="eval"):
    ...
    for batch in _iter_with_progress(loader, show_progress, desc=desc):
        ...
        mask = (targets != -100) & torch.isfinite(targets)
        if mask.sum() == 0:
            continue
...
def parse_args():
    p = argparse.ArgumentParser(description="ATLAS B-factor regression (minimal)")
    p.add_argument(
        "--h5",
        required=True,
        help="Path to ATLAS embedding H5 (e.g., vq_embed_atlas_lite.h5)",
    )
    p.add_argument(
        "--embeddings-dataset",
        default="/",
    ...
    p.add_argument(
        "--data-root",
        default=os.path.join(os.path.dirname(__file__), "struct_token_bench_release_data", "data", "physicochemical"),
...
def main():
    ...
    logger = logging.getLogger("atlas_bfactor_eval")
