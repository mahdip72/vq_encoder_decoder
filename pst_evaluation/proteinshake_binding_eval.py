...
@torch.no_grad()
def eval_auroc(model, loader, device, max_batches=None, show_progress=False, desc="eval"):
    model.eval()
    metric = AUROC(task="binary").to(device)
    total = 0
    pos = 0
    for step, batch in enumerate(_iter_with_progress(loader, show_progress, desc=desc)):
...
def parse_args():
    p = argparse.ArgumentParser(description="ProteinShake binding site AUROC eval (minimal)")
    p.add_argument(
        "--h5",
        required=True,
        help="Path to ProteinShake embedding H5 (e.g., vq_embed_proteinshake.h5)",
    )
    p.add_argument(
        "--embeddings-dataset",
        default="/",
...
def main():
    ...
    logger = logging.getLogger("proteinshake_binding_eval")
    ...
    test_ds = build_dataset("test", args, tokenizer, logger)
    ...
    for epoch in range(args.epochs):
        ...
        val_auroc = eval_auroc(
            model, val_loader, device, max_batches=val_batches, show_progress=args.progress, desc="val"
        )
        test_auroc = eval_auroc(
            model, test_loader, device, max_batches=test_batches, show_progress=args.progress, desc="test"
        )
        logger.info(
            f"[train] epoch={epoch+1} loss={loss:.4f} "
            f"val_auroc={val_auroc*100:.2f} "
            f"test_auroc={test_auroc*100:.2f}"
        )
    ...
    logger.info(f"[final] test AUROC% = {test_auroc*100:.2f}")
