...
class RepeatPredictor(nn.Module):
...
def build_dataset(split: str, args, tokenizer, logger):
    ...
    ds = InterProFunctionDataset(
        data_path=args.data_root,
        split=split,
        target_field="repeat_label",
        pdb_data_dir=None,
        tokenizer=tokenizer,
        logger=logger,
        cache=True,
...
def parse_args():
    p = argparse.ArgumentParser(description="InterPro repeats site AUROC eval (minimal)")
    p.add_argument(
        "--h5",
        required=True,
        help="Path to InterPro repeat H5 embeddings",
    )
    p.add_argument(
        "--embeddings-dataset",
        default="/",
    ...
    p.add_argument(
        ...
        help="Root containing interpro/processed_structured_* files",
    )
...
def main():
    ...
    logger = logging.getLogger("interpro_repeats_eval")
