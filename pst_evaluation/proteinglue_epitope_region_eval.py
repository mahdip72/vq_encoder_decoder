...
def build_dataset(split: str, args, tokenizer, logger):
    ...
    ds = ProteinGLUEEpitopeRegionDataset(
        data_path=args.data_root,
        split=split,
        target_field="epitope_label",
        pdb_data_dir=None,
        tokenizer=tokenizer,
        logger=logger,
        cache=True,
...
def parse_args():
    p = argparse.ArgumentParser(description="ProteinGLUE epitope region AUROC eval (minimal)")
    p.add_argument(
        "--h5",
        required=True,
        help="Path to ProteinGLUE embedding H5 (e.g., vq_embed_proteinglue_lite.h5)",
    )
    p.add_argument(
        "--embeddings-dataset",
        default="/",
...
def main():
    ...
    logger = logging.getLogger("proteinglue_epitope_region_eval")
