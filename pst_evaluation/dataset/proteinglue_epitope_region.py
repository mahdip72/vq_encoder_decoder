...
class ProteinGLUEEpitopeRegionDataset(Dataset):
    def __init__(
        self,
        data_path,
        split,
        ...
        raw_file = os.path.join(
            ...
            f"processed_structured_epitope_region_{self.split}",
        )
        if not os.path.isfile(raw_file):
            raise FileNotFoundError(f"Processed ProteinGLUE file not found: {raw_file}")
    ...
    def __getitem__(self, idx):
        ...
        label_res_idx = getattr(pdb_chain, "residue_index", None) if pdb_chain is not None else None
