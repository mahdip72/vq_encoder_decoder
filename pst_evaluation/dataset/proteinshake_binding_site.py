...
class ProteinShakeBindingSiteDataset(Dataset):
    def __init__(
        self,
        data_path,
        split,
        ...
        if not os.path.isfile(raw_file):
            raise FileNotFoundError(f"Processed ProteinShake file not found: {raw_file}")
    ...
    def __getitem__(self, idx):
        ...
        if labels is None or residue_index is None:
            return None
        if len(labels) != len(residue_index):
            return None
        ...
        if len(token_ids) == len(labels):
            ...
            aligned = self._align_by_residue_index(token_ids, token_res_idx, residue_index, labels)
            if aligned is None:
                return None
