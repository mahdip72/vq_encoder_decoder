...
class AtlasDataset(Dataset):
    ...
    def __init__(
        ...
        if target_field not in self.FULL_FIELD_MAPPING:
        ...
        raw_file = os.path.join(self.data_path, "atlas", f"processed_structured_{self.split}")
        if not os.path.isfile(raw_file):
            raise FileNotFoundError(f"Processed ATLAS file not found: {raw_file}")
    ...
    def _parse_pdb_chain(self, item):
        pdb_id = item.get("pdb_id")
        chain_id = item.get("chain_id")
        if not pdb_id:
            key = item.get("pdb_id_chain_id")
            if isinstance(key, str) and "_" in key:
                pdb_id, chain_id = key.split("_", 1)
            elif key:
                pdb_id = key
        return (str(pdb_id).lower() if pdb_id else None), chain_id

    def _prefilter_missing_h5(self):
        kept = []
        skipped = 0
        for item in self.data:
            pdb_id, chain_id = self._parse_pdb_chain(item)
            if not pdb_id:
                skipped += 1
                continue
    ...
    def _get_labels(self, item):
        ...
        if labels is None:
            annot_df = item.get("annot_df")
            if annot_df is None:
                return None
            col = self.FULL_FIELD_MAPPING[self.target_field]
            try:
                labels = np.asarray(annot_df[col].values, dtype=np.float32)
            except Exception:
                return None
            labels = labels * float(self.NORMALIZING_FACTOR[self.target_field])
    ...
    def __getitem__(self, idx):
        ...
        if label_res_idx is None:
            label_res_idx = item.get("residue_index")
