...
from visualization.main import compute_visualization_gvp
...
def train_loop(net, train_loader, epoch, **kwargs):
    ...
    for i, data in enumerate(train_loader):
        with accelerator.accumulate(net):
            ...
            masks = data['masks']
            optimizer.zero_grad()
            outputs, indices, commit_loss, gvp_embeddings= net(data)
...
def main(dict_config, config_file_path):
    ...
    for epoch in range(1, configs.train_settings.num_epochs + 1):
        ...
        if epoch % configs.visualization_settings.do_every == 0:
            ...
            compute_visualization_gvp(net, visualization_loader, result_path, configs, logging, accelerator, epoch)
